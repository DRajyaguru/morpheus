...
$fields  = array('c.category_id','c.category_name','c.category_name','c.category_img','c.display_order','c.status');
...
$columns = array(
    ...
    array('db' => 'category_name',	
	      'dt' => 1,
		  'formatter' => function( $d, $row ) {
			$category_img = str_replace(' ', '%20', $row['category_img']);  
		  	return '<div class="d-flex align-items-center"><a href="#" class="symbol symbol-50px"><span class="symbol-label" style="background-image:url(../assets/uploads/category/'.$category_img.');"></span></a><div class="ms-5"><a href="#" class="text-gray-800 text-hover-primary fs-5 fw-bolder mb-1" category-filter="category_name">'.$row['category_name'].'</a></div></div>';
		  }),
		  
	array('db' => 'display_order',	
	      'dt' => 2,		  
		  'formatter' => function( $d, $row ) {
		  	return '<div class="d-flex align-items-center"><input type="text" value='.$row['display_order'].' onkeyup=this.value=this.value.replace(/[^0-9]/g,"") class="form-control display_order_val" data-id="'.$row['category_id'].'" style="max-width: 55px;" maxlength="3" /></div>';
		  }),
